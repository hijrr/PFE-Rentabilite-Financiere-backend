...
def create_historique(historique: schemas.HistoriqueSalarieCreate, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
...
def get_historiques(db: Session = Depends(get_db),current_user: models.User = Depends(oauth2.get_current_user),
):
